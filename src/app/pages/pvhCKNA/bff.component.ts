...
import { OBKEYS, OORKEYS, OTRKEYS, SAKEYS, OLRKEYS, } from "../../../constants/columnKeys"; //need to replace with the keys belongs to the files
...
// newly important components
import { comparisonAOB_AASModelComponent } from "./comparison-AOB_AAS-model/comparison-AOB_AAS-model.component";
import { ComparisonAOB_PSSModelComponent } from "./comparison-AOB_PSS-model/comparison-AOB_PSS-model.component";
import { ComparisonBS_PSSModelComponent } from "./comparison-BS_PSS-model/comparison-BS_PSS-model.component";
...
export class bffComponent_CKNA {
  ...
  @ViewChild(ComparisonAOB_PSSModelComponent) AOB_PSSComparisonModel: ComparisonAOB_PSSModelComponent;
  @ViewChild(ComparisonBS_PSSModelComponent) BS_PSSComparisonModel: ComparisonBS_PSSModelComponent;
  @ViewChild(comparisonAOB_AASModelComponent) AOB_AASComparisonModel: comparisonAOB_AASModelComponent;
  ...
  // variables for pvh update - start
  isBSProcessing: boolean;
  isPSSProcessing: boolean;
  isAOBProcessing: boolean;
  isAASProcessing: boolean;
  ...
  // ngOnInit(){
  //   this.disabledButtonOne = false;
  //   this.disabledButtonTwo = true;
  //   this.disabledButtonThree = true;
  // }

  onFileChange(fileType, files) {
    switch (fileType) {
      case "BSFile":
        this.BSFileObject = files[0];
        break;
      case "PSSFile":
        this.PSSFileObject = files[0];
        break;
      case "AOBFile":
        this.AOBFileObject = files[0];
        break;
      case "AASFile":
...
// New codes only for update files - start

async onUploadBSFile() {
  
  // alert("Function is Working");
  this.isBSProcessing = true;

  if(this.BSFileObject == null){
    this.toastr.warning("", "Buy Sheet is missing!");
  }

  const BSFileJson = await this.readExcelFile(this.BSFileObject, OBKEYS);
  const finalBSFileData = []
  
  for(const row of BSFileJson){
    finalBSFileData.push(row);
  }

  const dbInserResponse = await this.DbconService.addBulk("BuySheet", finalBSFileData);
 
  this.toastr.success("", "Buy Sheet is Uploaded");
  this.isBSProcessing = false;
}

async onUploadPSSFile() {
  
  // alert("Function is Working");
  this.isPSSProcessing = true;

  if(this.PSSFileObject == null){
    this.toastr.warning("", "PVH Shipment Status is missing!");
  }

  const PSSFileJson = await this.readExcelFile(this.PSSFileObject, OBKEYS);
  const finalPSSFileData = []
  
  for(const row of PSSFileJson){
    finalPSSFileData.push(row);
  }

  const dataInsertResponse = await this.DbconService.addBulk("PVHSSFile", finalPSSFileData);

  this.toastr.success("", "PVH Shipment Status is Uploaded");
  this.isPSSProcessing = false;
}

async onUploadAOBFile() {
  
  // alert("Function is Working");
  this.isAOBProcessing = true;

  if(this.AOBFileObject == null){
    this.toastr.warning("", "APL-Order Book is missing!");
  }

  const AOBFileJson = await this.readExcelFile(this.AOBFileObject, OBKEYS);
  const finalAOBFileData = []
  
  for(const row of AOBFileJson){
    finalAOBFileData.push(row);
  }

  const dataInsertResponse = await this.DbconService.addBulk("APLOBFile", finalAOBFileData);

  this.toastr.success("", "APL-Order Book is Uploaded");
  this.isAOBProcessing = false;
}

async onUploadAASFile() {
  
  // alert("Function is Working");
  this.isAASProcessing = true;

  if(this.AASFileObject == null){
    this.toastr.warning("", "AA Sheet is missing!");
  }

  const AASFileJson = await this.readExcelFile(this.AASFileObject, OBKEYS);
  ...
  const dataInsertResponse = await this.DbconService.addBulk("AASheet", finalAASFileData);

  this.toastr.success("", "AA Sheet is Uploaded");
  this.isAASProcessing = false;
}
...
BSvsPSSbuttonActive() {
  ...
  if(this.PSSFileObject == null ){
    if(this.BSFileObject == null){
      this.toastr.warning("", "Upload Buy Sheet and PVH Shipment Status files");
      //alert("Upload Buy Sheet and PVH Shipment Status files");
    }else{
      this.toastr.warning("", "Upload PVH Shipment Status file");
      // alert("Upload PVH Shipment Status file");
    }
  }else{
    if(this.BSFileObject == null){
      this.toastr.warning("", "Upload Buy Sheet file");
      // alert("Upload Buy Sheet file");
    }else{
      this.toastr.success("", "Files have uploaded");
      // alert("Files have uploaded");
    }
  }
}

AOBvsPSSbuttonActive() {
  
  if(this.PSSFileObject == null ){
    if(this.AOBFileObject == null){
      this.toastr.warning("", "APL-Order Book and PVH Shipment Status files");
      //alert("APL-Order Book and PVH Shipment Status files");
    }else{
      this.toastr.warning("", "Upload PVH Shipment Status file");
      // alert("Upload PVH Shipment Status file");
    }
    ...
    if(this.AOBFileObject == null){
      this.toastr.warning("", "APL-Order Book file");
      // alert("APL-Order Book file");
    }else{
      this.toastr.success("", "Files have uploaded");
      // alert("Files have uploaded");
    }
...
AOBvsAASbuttonActive() {
  
  if(this.AASFileObject == null ){
    if(this.AOBFileObject == null){
      this.toastr.warning("", "APL-Order Book and AA Sheet files");
      //alert("APL-Order Book and AA Sheet files");
    }else{
      this.toastr.warning("", "Upload AA Sheet file");
      // alert("Upload AA Sheet file");
    }
  }else{
    if(this.AOBFileObject == null){
      this.toastr.warning("", "APL-Order Book file");
...
// create AOB Unique Key function

async createAOBUniqueKey() {
...
// New codes only for update files - end

// Old codes from the previous order list verification
  ...
  async onUploadOBFile() {
    ...
    if(this.OBFileObject == null){
      this.toastr.success("", "OB file is missing!"); 
    }
